...
## Last Updated
2026-05-11 (latest). **Internal JIRA wiki ingest — `analysis-for-query-processing-` cluster (8 pages, 20 images, 8 PDFs, 3 PPTX).** 박세훈/채광수 작성 한국어 분석서를 wiki/sources/ 아래 9 페이지 (hub + 8 sub) 로 정리. 단계별 page 와 [[components/parser]] / [[components/semantic-check]] / [[components/optimizer]] / [[components/optimizer-rewriter]] / [[components/xasl-generation]] / [[components/xasl]] / [[components/query-executor]] / [[components/query-manager]] / [[components/query-fetch]] 양방향 cross-link. 다이어그램 20개를 multimodal 해석하여 본문에 임베드 + 구조 디코드. 핵심 인사이트: (1) WHERE_RANGE/WHERE_KEY/WHERE_PRED 3-way 분할이 `qo_analyze_term()` 에서 결정되어 XASL `regu_list_range/key/pred` 로 분배, (2) APTR=uncorrelated 1회 실행 + temp 캐시, DPTR=correlated 매-row 재실행, SCAN_PTR=join inner (mainblock 동행), (3) Tempfile thread-affinity: `file_Tempcache.tran_files` 가 thread-local → parallel query 는 external-sort preserve 패턴 또는 `db_change_private_heap(thread_p, 0)` 필요, (4) CNF 100항 초과 시 변환 포기 → 인덱스 스캔 비후보, (5) Optimizer first-node 선별이 `cost` 우선 (`card` 가 더 적절하다고 분석서 author 직접 지적). Hub: [[sources/qp-analysis]]. .raw/qp-analysis-*.md + .raw/qp-pdfs/ + _attachments/qp-analysis/ 보존. settings.local.json 에 playwright + JIRA curl + autoMode.allow 규칙 추가.

> [!update] **Baseline drift**: source HEAD `bac5eb16` 가 baseline `05a7befd` 의 직계 후손 아님 (divergent branch). 본 ingest 는 개념-중심이라 영향 미미; **baseline 미bump** (어느 branch 인지 사용자 확인 필요).

## Earlier Updates
2026-05-08 (prev latest). **Two-PR ingest: #6930 + #7102 (both case-c).** PR #6930 — `lock_manager.c` init/finalize refactor: introduces `LK_CONFIG` (16 fields) and `LK_INIT_STATE` (4 flags) on `lk_Gl`; replaces `.bss` `TWFG_edge_block[]` / `victims[]` with heap-owned `lk_Gl.TWFG_edge_storage` / `lk_Gl.victims`; finalize gated by `init_state` flags; daemon ptr null-out on destroy; fixes partial-init mutex-destroy bug. PR #7102 — `db_get_char` 1-arg cleanup + SWAR ASCII fast path in `intl_count_utf8_chars` / `intl_count_utf8_bytes` / `intl_check_utf8`; precision-truncate fast paths in `db_string_truncate` and loaddb converters; loaddb `val.domain.char_info.length = precision` semantic correction (was `char_count`). Reconciled: [[components/lock-manager]], [[components/deadlock-detection]], [[components/db-value]], [[components/base]], [[components/cas]], [[components/loaddb-executor]]. Baseline `0be6cdf6` → `5e12a293` → `05a7befd` (transitively absorbs PR #7145 `.travis.yml` removal).
