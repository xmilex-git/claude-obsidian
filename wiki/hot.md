...
## PR Ingest
Merged upstream PRs are documented under `wiki/prs/PR-NNNN-<slug>.md` (template: `_templates/pr.md`; index: [[prs/_index|PRs]]). Only `state: MERGED` PRs are accepted; merged PRs newer than baseline trigger the reconciliation + baseline-bump flow. Protocol: `CLAUDE.md` § "PR Ingest (merged PRs only)". User says "ingest PR #NNNN" → fetch via `gh`, classify vs baseline, reconcile, bump.
