...
**All wiki content under `wiki/` (outside `_legacy/`) is anchored to CUBRID commit `05a7befd8b714811632a16a97d3683ab3b397a0f`.** Every claim, file path, line number, and structural observation reflects the source tree at that commit.
...
1. Check the current HEAD of the CUBRID source tree:
   ...
   ```
2. If HEAD == `05a7befd8b714811632a16a97d3683ab3b397a0f`, proceed normally.
3. If HEAD is **newer** (i.e. `git -C ~/dev/cubrid/ merge-base --is-ancestor 05a7befd8b714811632a16a97d3683ab3b397a0f HEAD` exits 0), do this before writing anything:
   a. Compute the delta for the path you are about to ingest/update:
      ```
      git -C ~/dev/cubrid/ log --oneline 05a7befd8b714811632a16a97d3683ab3b397a0f..HEAD -- <path>
      git -C ~/dev/cubrid/ diff --stat 05a7befd8b714811632a16a97d3683ab3b397a0f..HEAD -- <path>
      ```
...
3. **Deep code analysis.** Not just `gh pr diff`. For every file in `files_changed`:
   ...
   **Scale rule — mandatory subagent dispatch for large PRs.** If the PR has **>20 files changed** or **>3,000 LOC diff** or **>1 file over 500 LOC**, do NOT attempt deep analysis in the main thread — you will sample rather than read, and the user will be right to push back. Instead, dispatch **parallel subagents** (one `Agent` tool call per file cluster, all in a single message so they run concurrently) with focused prompts and a strict "only NEW findings beyond the review doc" rubric. Suggested clustering: (a) largest new file + its task/manager wrappers, (b) largest new file #2, (c) remaining new files grouped by conceptual area, (d) heavily-modified existing files (result handlers, checkers, trace handlers), (e) C-side integration (scan_manager, query_executor, parser, optimizer, system_parameter, storage helpers). Each subagent reports back a bulleted list of invariants, wiki gaps, subtle changes, and code concerns — integrate all findings into the PR page under a "## Deep analysis — supplementary findings" section. Smaller PRs (≤20 files, ≤3000 LOC) can be read in the main thread.
